param(
    ...
    [string] $OUSuffix, #the part after the "...,DC=avdsandbox,DC=org" so e.g. "OU=HostPool1,OU=AVD"

    [Parameter(Mandatory = $True, Position = 2)]
    [string] $storageAccountName,

    [Parameter(Mandatory = $True, Position = 3)]
    [string] $profileShareName
)
...
$FSLogixRegKeys = @{
    ...
    @{
        Type  = "MultiString"
        Value = "\\$storageAccountName.file.core.windows.net\$profileShareName"
    }
...
#Get-GPRegistryValue -Name $gpoName -Key "HKEY_LOCAL_MACHINE\Software\Policies\Microsoft\Windows Defender\Exclusions\Paths" 
...
%Windir%\TEMP\*.VHD,
%Windir%\TEMP\*.VHDX,
\\$storageAccountName.file.core.windows.net\$profileShareName\*\*.VHD,
\\$storageAccountName.file.core.windows.net\$profileShareName\*\*.VHDX
"@
